...
import os 
os.makedirs("freedrag_results",exist_ok=True)
...
def on_drag(model, points, mask, max_iters,latent,sample_interval,l_expected,d_max,save_video,add_watermark, seed):
    ...
    images_total = []
    b = 1
    for img_show, current_target, step_number,full_res, latent_optimized in free_drag(model.g, points, mask[:,:,0], handle_size, train_layer_index, latent,max_iters, l_expected, d_max,sample_interval, device=latent.device):
        ...
        image_name = f'./freedrag_results/seedid-{seed}_freameID-{b}.png'
        image_name_makred = f'./freedrag_results/seedid-{seed}_freameID-{b}_makred.png'
        b = b + 1
        #img_show.save(image_name)
        print(img_show.shape)
        print(image_clear.shape)
        #torchvision.utils.save_image(img_show,image_name)
        image_clear = cv2.cvtColor(image_clear, cv2.COLOR_RGB2BGR)
        cv2.imwrite(image_name,image_clear)


def add_points_to_image(image, points, size=1, color="red"):
...
def new_image(model,seed=-1): #该部分更换为llf的stylegan2随机数
    if seed == -1:
...
with demo:
    ...
    button_drag.click(on_drag, inputs=[model, points, mask, max_step,latent,sample_interval,l_expected,d_max,if_save_video,add_watermark,seed], \
    ...
    model_name.change(new_model,inputs=[model_name],outputs=[model,res,l_expected,d_max]).then (new_image, inputs = [model,seed],outputs = [image, image_clear, latent,seed]).then (reset_all,inputs=[image_clear,mask],outputs=[points,target_point,image,mask_show,mask,add_watermark]) 
